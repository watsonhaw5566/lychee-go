...
本模块复用 `internal/cache` 模块，支持多种缓存驱动：

| 驱动类型 | 锁机制 | 适用场景 |
|----------|--------|----------|
| `memory` | 内存锁 (`sync.Map`) | 单机、开发环境 |
| `file` | 内存锁 (`sync.Map`) | 单机、文件持久化 |
| `redis` | 分布式锁 (`SetNX`) | 分布式、生产环境 |

缓存驱动通过 `config/config.yml` 中的 `cache.driver` 配置项指定。模块会自动根据驱动类型选择合适的锁机制，确保并发安全。
